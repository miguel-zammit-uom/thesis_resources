...
class ConfigReader:
    ...
    def __init__(self, configfile):
        ...
        self.max_generations = np.int(self.config.get('Genetic Algorithm Settings',
                                                      'max_generations'))
        self.pop_size = np.int(self.config.get('Genetic Algorithm Settings',
                                               'pop_size'))
        self.imputation_threshold = np.float(self.config.get('Genetic Algorithm Settings',
                                                             'imputation_obj_val_threshold'))
        self.clf_threshold = np.float(self.config.get('Genetic Algorithm Settings',
                                                      'clf_obj_val_threshold'))
        self.rndm_seed = np.float(self.config.get('Genetic Algorithm Settings',
                                                  'rndm_seed'))
        self.num_replacement_fronts = np.int(self.config.get('Genetic Algorithm Settings',
                                                             'num_replacement_fronts'))
        self.c_r = np.int(self.config.get('Genetic Algorithm Settings',
                                          'c_r'))
        self.run_name = np.str(self.config.get('Genetic Algorithm Settings',
                                               'run_name'))
        ...
        self.element_features_list = np.str(self.config.get('Dataset Settings',
                                                            'element_features_list')).split(",")
        self.log_absolute_conversion = str2bool(np.str(self.config.get('Dataset Settings',
                                                                       'log_absolute_conversion')))

        # Imputation Settings
        self.max_clusters = np.int(self.config.get('Imputation Settings',
                                                   'max_clusters'))
        self.fuzziness_min = np.float(self.config.get('Imputation Settings',
                                                      'fuzziness_min'))
        self.fuzziness_max = np.float(self.config.get('Imputation Settings',
                                                      'fuzziness_max'))
        self.cluster_validity_fn = np.int(self.config.get('Imputation Settings',
                                                          'cluster_validity_fn'))
