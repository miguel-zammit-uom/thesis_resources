...
class InitialPopulationGeneration:
    ...
    def __init__(self, pop_size, max_clusters, accepted_ranges):
        """Randomly generate the first generation of chromosomes to start off the GA. 
        Note that the chromosomes generated here won't be 'full', as the cluster centres 
        will be determined after the first imputation of the missing values as described
        in DatasetImputation (imputation_and_fitness.py). At this stage for the imputation
        parameters, the chromosomes will have the number of clusters and the fuziness 
        parameter."""
...
def _chromosome_history_storage(population, fitnesses, metrics, generation, num_features, output_csv_filename):
    if generation == 1:
        ...
        csv_file.write('num_clusters,centres,fuzziness,num_estimators,max_depth,lr,subsample,col_sample,'
                       'gamma,reg_lambda,reg_alpha,Imputation_Fitness,Clf_Fitness,Clf_Accuracy,Clf_Accuracy_err'
