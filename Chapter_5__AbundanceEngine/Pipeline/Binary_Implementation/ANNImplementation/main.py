...
prev_gen_avg_fitness, fitnesses, accuracy_scores, f1_scores, recall_scores, precision_scores \
        = {}, [], [], [], [], []
gen_1_mean_imp_x_train, gen_1_mean_imp_x_test = np.array([]), ([])
final_pareto_front = []
top_clf_fit_reached,  earlystopping = 0, 0   # CHANGE FOR ANN GA, INCLUDED AN EARLY STOPPING STYLE CRITERIA
for generation in range(1, configreader.max_generations + 1):
    ...
    # Calculate Fitnesses
    fitnesses, full_chromosome_population, accuracy_scores, accuracy_std, f1_scores = [], [], [], [], []
    f1_std, recall_scores, recall_std, precision_scores, precision_std = [], [], [], [], []
    for chromosome in tqdm(population, desc='        Fitness Calculation'):
        ...
        fitnesses.append([fitness_calc.fitness_1, fitness_calc.fitness_2])
        accuracy_scores.append(fitness_calc.accuracy_score[0])
        accuracy_std.append(fitness_calc.accuracy_score[1])
        f1_scores.append(fitness_calc.f1_score[0])
        f1_std.append(fitness_calc.f1_score[1])
        recall_scores.append(fitness_calc.recall_score[0])
        recall_std.append(fitness_calc.recall_score[1])
        precision_scores.append(fitness_calc.precision_score[0])
        precision_std.append(fitness_calc.precision_score[1])
    ...
    # to be replaced
    population, fitnesses, accuracy_scores, f1_scores, recall_scores, precision_scores \
        = np.array(full_chromosome_population), np.array(fitnesses), np.array(accuracy_scores), np.array(f1_scores), \
        np.array(recall_scores), np.array(precision_scores)
    accuracy_std, f1_std, recall_std, precision_std \
        = np.array(accuracy_std), np.array(f1_std), np.array(recall_std), np.array(precision_std)
    ...
    _chromosome_history_storage(population=population, fitnesses=fitnesses,
                                metrics=np.transpose(np.array([accuracy_scores, accuracy_std, f1_scores, f1_std,
                                                               recall_scores, recall_std, precision_scores,
                                                               precision_std])),
    ...
    if generation == 1:
        ...
        acc_avg, acc_max = np.mean(accuracy_scores[~np.isnan(accuracy_scores)]), \
                           np.max(accuracy_scores[~np.isnan(accuracy_scores)])
        ...
        # CHANGE FOR ANN GA, INCLUDED AN EARLY STOPPING STYLE CRITERIA
        if maxs[1] > top_clf_fit_reached:
            top_clf_fit_reached = maxs[1]
            earlystopping = 0
        else:
            earlystopping += 1
        ...
        stopping_criteria = StoppingCriteriaCheck(first_pareto_front_size=len(tourn_selection.first_pareto_front),
                                                  ...
                                                  pop_size=configreader.pop_size,
...
_final_pareto_fronts_storage(population=population, fitnesses=fitnesses,
                             metrics=np.transpose(np.array([accuracy_scores, f1_scores,
