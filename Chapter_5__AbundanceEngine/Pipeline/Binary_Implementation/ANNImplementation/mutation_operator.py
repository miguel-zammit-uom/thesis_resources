...
def _mutation(chromosome, accepted_ranges):
    ...
    for i in mutated_genes_index:
        ...
        else:   # One of the final six hyper parameters
            if i == len(chromosome) - 9:    # fuzziness
                new_gene = np.random.uniform(low=accepted_ranges['fuzz_min'], high=accepted_ranges['fuzz_max'])
            elif i == len(chromosome) - 8:  # num_hdn_layers
                new_gene = np.random.randint(low=accepted_ranges["num_hdn_layers_min"],
                                             high=accepted_ranges["num_hdn_layers_max"])
            elif i == len(chromosome) - 7:  # num_neurons_first
                new_gene = np.random.randint(low=accepted_ranges["num_neurons_first_min"],
                                             high=accepted_ranges["num_neurons_first_max"]
                                             )
            elif i == len(chromosome) - 6:  # reg_l1
                new_gene = np.random.choice([10**i for i in range(-accepted_ranges["reg_l1_smallest_order"], 1)])
            elif i == len(chromosome) - 5:  # reg_l2
                new_gene = np.random.choice([10**i for i in range(-accepted_ranges["reg_l2_smallest_order"], 1)])
            elif i == len(chromosome) - 4:  # dropout
                new_gene = np.random.uniform(low=accepted_ranges["dropout_min"], high=accepted_ranges["dropout_max"])
            elif i == len(chromosome) - 3:  # activation_fn
                new_gene = np.random.randint(low=1, high=5)
            elif i == len(chromosome) - 2:  # optimizer
                new_gene = np.random.randint(low=1, high=5)
            elif i == len(chromosome) - 1:  # num_epochs
                new_gene = np.random.randint(low=accepted_ranges["num_epochs_min"],
                                             high=accepted_ranges["num_epochs_max"])
