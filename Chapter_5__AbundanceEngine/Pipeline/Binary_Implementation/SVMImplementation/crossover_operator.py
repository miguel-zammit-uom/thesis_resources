...
def _crossover_operation(parent_chromosomes):
    ...
    parent1_centres, parent1_fixed = parent1[:-6], parent1[-6:]
    parent2_centres, parent2_fixed = parent2[:-6], parent2[-6:]
    random_vector_centres, random_vector_fixed = random_vector[:-6], random_vector[-6:]

    child_chromosome = []
    for i in range(child_chromosome_len):
        if i < child_chromosome_len-6:     # The cluster centres genes
            if i < random_vector_len-6:    # The centre genes for which a random number in the vector was generated
...
def _crossover_selection(population, fitnesses, C_r):
    ...
    if len(population) == len(fitnesses):
        ...
        # Select Best Performer (Max Value) for Obj 1 and Obj 2
        best_performer_obj1_index = np.argmax(selected_crossover_fitnesses[:, 0])
        best_performer_obj2_index = np.argmax(selected_crossover_fitnesses[:, 1])
        ...
        # Select Worst Performer (Min Value) for Obj 1 and Obj 2
        worst_performer_obj1_index = np.argmin(selected_crossover_fitnesses[:, 0])
        worst_performer_obj2_index = np.argmin(selected_crossover_fitnesses[:, 1])
