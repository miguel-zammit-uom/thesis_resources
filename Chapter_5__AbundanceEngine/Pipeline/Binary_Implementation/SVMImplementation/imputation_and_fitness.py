...
class FitnessCalculation:
    ...
    def __init__(self, chromosome, x_train_imputed, y_train, x_test_imputed, x_train_cluster_labels,
        ...
        self.cluster_centres = chromosome[:-6]
        self.fuzziness = chromosome[-6]
        self.svm_parameters = chromosome[-5:]
        ...
        self.fitness_2, self.accuracy_score, self.f1_score, self.recall_score, self.precision_score \
            = self._objective_fn_2(x_train_imputed=x_train_imputed, y_train=y_train,
                                   svm_hyperparams=self.svm_parameters,
    ...
    def _objective_fn_2(self, x_train_imputed, y_train, svm_hyperparams, cross_val, clf_metric, num_datasubsamples):
        C, K_r, gamma, d, r = svm_hyperparams

        error = False

        if K_r == 1:  # Linear Kernel Function
            clf = Pipeline([
                ("scaler", StandardScaler()),
                ("svm_clf", SVC(kernel='linear', C=C, max_iter=-1, class_weight='balanced'))
            ])
        elif K_r == 2:  # Radial Kernel Function
            clf = Pipeline([
                ("scaler", StandardScaler()),
                ("svm_clf", SVC(kernel='rbf', C=C, gamma=gamma, max_iter=-1, class_weight='balanced'))
            ])
        elif K_r == 3:  # Polynomial Kernel Function
            clf = Pipeline([
                ("scaler", StandardScaler()),
                ("svm_clf", SVC(kernel='poly', C=C, gamma=gamma, degree=d, coef0=r, max_iter=-1,
                                class_weight='balanced'))
            ])
        elif K_r == 4:  # Sigmoid Kernel Function
            clf = Pipeline([
                ("scaler", StandardScaler()),
                ("svm_clf", SVC(kernel='sigmoid', C=C, gamma=gamma, coef0=r, max_iter=-1, class_weight='balanced'))
            ])
        else:
            error = True
            clf = None
            print("RUNTIME ERROR: Chromosome choice of Kernel function is invalid")
        ...
        for sub_sample in data_subsamples:
            # np.savetxt('/users/mzammit/Abundance_clf_MOO_GA_Run_28_03_23/GA_with_XGB/Cluster_Pipeline/subsample.csv',
            #            sub_sample[0], delimiter=",")
