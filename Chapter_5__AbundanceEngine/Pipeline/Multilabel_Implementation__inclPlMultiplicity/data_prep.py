...
class DataPreparation:
    ...
    def __init__(self, dataset_path, element_features_list):
        ...
        self.features_list.append('Rocky_Host')
        self.features_list.append('Multiplanet_System')

        self.train_starname_list = self.traindata['f_preferred_name'].to_numpy()
        self.x_train = self.traindata[self.features_list].to_numpy()
        #   Convert ytrain from a single element array to one that is 4-element (shape: (samples, labels) )
        self.y_train = np.array([self.traindata['Jupiter_Host'].to_numpy(),
                                 self.traindata['Rocky_Host'].to_numpy(),
                                 self.traindata['Multiplanet_System'].to_numpy()
                                 ]).transpose()
