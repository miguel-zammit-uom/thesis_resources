...
class TournamentSelection:
    ...
    def __init__(self, population, fitnesses, num_replacement_fronts):
        ...
        In systems with small number of fronts, the num_replacement_fronts selects the entire population. This will
    	lead to overflushing the population with a copy of repeated fronts, passed through the replacement and
        mutation modules"""
        ...
        # Selection of first pareto front
        self.first_pareto_front = self.pareto_fronts[0]

        # Selection of Replacement fronts
        if len(self.pareto_fronts) > num_replacement_fronts:
            self.replacement_fronts = flatten(self.pareto_fronts[-num_replacement_fronts:])
        elif len(self.pareto_fronts) == num_replacement_fronts:
            self.replacement_fronts = flatten(self.pareto_fronts[-(num_replacement_fronts-1):])
        elif len(self.pareto_fronts) == 1:
            self.replacement_fronts = []
        else:
            self.replacement_fronts = flatten(self.pareto_fronts[-1:])

        # Selection of Mutation fronts
